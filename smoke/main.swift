...
await expectOk("sanctions", { try await parse.sanctions("AEROCARIBBEAN AIRLINES") }) { $0.sanctioned && $0.matches.first?.list == "sdn" ? nil : "expected sdn match" }
await expectOk("sanctions clean", { try await parse.sanctions("Jane Smith") }) { !$0.sanctioned && $0.matches.isEmpty ? nil : "expected no match" }
