...
@Suite struct PostalMetros {
	@Test func observationStatesSurviveEveryPostalResponse() async throws {
		let fields = ["", #", "metros":null"#, #", "metros":[]"#, #", "metros":[{"code":"12345","name":"Example area","type":"future-area-type","share":0.75,"residential_share":0,"business_share":1,"other_share":null,"future":true}]"#]
	...
	@Test func missingSharesRemainUnknown() async throws {
		let body = #"{"postal":"12345","country":"US","metros":[{"code":"12345","name":"Example area","type":"metropolitan"}]}"#
		let result = try await makeClient(StubTransport(body: body)).postal("12345")
		let metro = try #require(result.metros?.first)
