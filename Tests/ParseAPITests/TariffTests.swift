...
@Suite struct TariffTests {
	@Test func originalMethodReferencesRemainCallable() async throws {
		let stub = StubTransport([(200, #"{"hts":"0101","description":"Horses","lineage":[],"general":null,"revision":"fixture"}"#, [:]), (200, #"{"q":"horses","revision":"fixture","lines":[]}"#, [:])])
		let client = try makeClient(stub)
		let lookup: (String, Bool, String?) async throws -> Tariff = client.tariff
		let search: (String) async throws -> TariffSearch = client.tariffSearch
		#expect(try await lookup("0101", false, nil).hts == "0101")
		#expect(try await search("horses").lines.isEmpty)
	}
