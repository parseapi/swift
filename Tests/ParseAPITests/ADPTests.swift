...
@Suite struct ADPContracts {
	...
	@Test func nameDepth() async throws {
		...
		let rich = try decoder.decode(Name.self, from: Data(#"{"name":"Fixture","valid":true,"deep":{"gender":null,"salutation":null}}"#.utf8))
		#expect(core.deep == nil)
		#expect(locked.deep != nil && locked.deep?.gender == nil && locked.deep?.salutation == nil)
		#expect(rich.deep != nil && rich.deep?.gender == nil && rich.deep?.salutation == nil)
		for depth in [false, true] {
			let stub = StubTransport(body: #"{"name":"Fixture","valid":true,"deep":{"gender":null,"salutation":null}}"#)
