...
public final class ParseAPI: Sendable {
	...
	public func name(_ name: String, deep: Bool) async throws -> Name {
		try await self.name(name, country: nil, deep: deep)
	}

	/// Country is an ISO2 context for gender, not a nationality claim.
	public func name(_ name: String, country: String?, deep: Bool = false) async throws -> Name {
	...
	/// adds numbering-plan location context. Carrier, caller, and HLR are separate metered lookups.
	...
	public func carrier(_ number: String, country: String? = nil, deep: Bool = false) async throws -> Carrier {
		try await get("/carrier/\(enc(number))", query: [("country", country)] + deepQuery(deep))
	}
	...
	public func companySearch(query: String? = nil, domain: String? = nil, ticker: String? = nil,
		...
		try await companySearch(query: query, domain: domain, ticker: ticker, identifier: identifier,
			country: country, exchange: exchange, authority: authority, limit: limit, cursor: cursor,
			deep: deep, industry: industry, industryType: industryType, registrationAuthority: nil)
	}

	/// Discover or narrow by selected registration evidence. Exact form/status strings require registrationAuthority.
	/// Status is an administrative source value, not evidence of current business activity.
	/// The required registrationAuthority label preserves both earlier function-reference signatures.
	public func companySearch(query: String? = nil, domain: String? = nil, ticker: String? = nil,
		identifier: String? = nil, country: String? = nil, exchange: String? = nil,
		authority: String? = nil, limit: Int? = nil, cursor: String? = nil, deep: Bool = false,
		industry: String? = nil, industryType: String? = nil, registrationAuthority: String?,
		registrationForm: String? = nil, registrationStatus: String? = nil) async throws -> CompanySearch {
		try await get("/company", query: [("q", query), ("domain", domain), ("ticker", ticker),
			...
			("industry", industry), ("industry_type", industryType),
			("registration_authority", registrationAuthority), ("registration_form", registrationForm),
			("registration_status", registrationStatus)] + deepQuery(deep))
	}
